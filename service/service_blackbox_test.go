...
import (
	...
	"github.com/fabric8-services/fabric8-cluster-client/service"
...
var clusterURL = "http://localhost:8080"

type ClusterServiceTestSuite struct {
	testsuite.UnitTestSuite
	clusterSvc service.Service
}

func (s *ClusterServiceTestSuite) SetupSuite() {
	s.UnitTestSuite.SetupSuite()
	var err error
	s.clusterSvc, err = service.NewClusterService(clusterURL)
...
func (s *ClusterServiceTestSuite) TestClustersUser() {
	...
	s.T().Run("ok", func(t *testing.T) {
		...
		gotResp, err := s.clusterSvc.UserClusters(ctx)
	...
	s.T().Run("unauthorized", func(t *testing.T) {
		...
		_, err := s.clusterSvc.UserClusters(ctx)
